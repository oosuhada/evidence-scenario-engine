...
import { Check, GitCompareArrows, Plus, RotateCcw, Save, ShieldAlert, Trash2 } from 'lucide-react';
import type { ScenarioSet, StrategyDecision } from '../../decision-model/types';
import { calculateStabilityMap, compareScenarioSets } from '../../scenario-engine/engine';
...
export function ScenarioWorkbench({
  ...
  return <section className="analysis-card scenario-workbench" aria-labelledby="scenario-workbench-heading">
    ...
    <div className="stability-map-section">
      ...
      {stability.cells.length ? <div className="stability-map-grid">{stability.cells.map((cell) => <article key={`${cell.xState}-${cell.yState}`} className={cell.leaderId !== stability.baselineLeaderId ? 'switch' : ''}><span>{cell.xState.toUpperCase()} × {cell.yState.toUpperCase()}</span><strong>{cell.leaderName}</strong><small>{stability.xAssumption?.name}: {cell.xValue.toFixed(1)} · {stability.yAssumption?.name}: {cell.yValue.toFixed(1)}</small><b>{cell.leaderId !== stability.baselineLeaderId ? 'LEADER SWITCH' : 'BASE LEADER'} · {cell.guardrailPass ? 'PASS' : 'BREACH'}</b></article>)}</div> : <p className="scenario-empty">Add at least two assumptions to calculate a stability map.</p>}
    </div>
